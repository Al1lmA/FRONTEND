...
const fetchRequestData = async (filters: any, session_id: any, setBreachesData: any, setError: any, setLoadedOnce: any, setIsLoading: any) => {
  ...
  const staticColumns = [
    ...
    {
        Header: "Услуги",
        accessor: "BankServices",
        Cell: ({ value }) => { return value?.map(service => service.title).join(', ') }
    },
    {
        Header: "Рейтинг",
        accessor: "rating",
    ...
    const COLUMNS = useMemo(() => {
      ...
      useEffect(() => {
        ...
        const fetchDataInterval = async () => {
            await fetchRequestData(filters, session_id, setBreachesData, setError, setLoadedOnce, setIsLoading);
        };
        ...
        if (is_moderator) {
            ...
            fetchRequestData(filters, session_id, setBreachesData, setError, setLoadedOnce, setIsLoading);
        }
